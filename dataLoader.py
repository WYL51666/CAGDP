...
def buildIndex(data):
    ...
    for line in open(data):
        ...
        if len(line.strip()) == 0:
            continue
        # chunks = line.strip().split(',') #原始
        chunks = line.strip().split() #meMe
        for chunk in chunks:
            try:
                if len(chunk.split()) == 1:
                    # user, timestamp = chunk.split()#原始
                    user, timestamp = chunk.split(',')#meMe
...
def Split_data(data_name, train_rate=0.8, valid_rate=0.1, load_dict=False):
    ...
    if not load_dict:
        ...
        with open(options.u2idx_dict, 'wb') as handle:
            pickle.dump(u2idx, handle, protocol=2)
        with open(options.idx2u_dict, 'wb') as handle:
            pickle.dump(idx2u, handle, protocol=2)
    else:
        with open(options.u2idx_dict, 'rb') as handle:
            u2idx = pickle.load(handle)
        with open(options.idx2u_dict, 'rb') as handle:
        ...
        for chunk in chunks:
            try:
                ...
                # Android,Christianity
                    ...
                    timestamplist.append(int(timestamp))#f
            except:
                print(chunk)

            userlist.append((u2idx[user]))
            timestamplist.append(timestamp)#f
